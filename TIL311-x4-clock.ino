...
void setup() {
  ...
  for(int a = 0; a < 4; a++){pinMode(latches[a], OUTPUT); digitalWrite(latches[a], HIGH);} //set data latches outputs
  for(int a = 0; a < 4; a++){pinMode(blanking[a], OUTPUT); analogWrite(blanking[a], 150);} // set blanking/PWM outputs
